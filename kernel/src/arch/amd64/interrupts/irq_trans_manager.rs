...
use crate::arch::amd64::{interrupts::idt::ISR_COUNT, ipc::{port::Port}, scheduler::task::TaskId};
...
impl IDTTransferManager {
    ...
    pub fn bind_irq(&self, task_id: TaskId, port_arc: Arc<Port>, vector: u8, key: u64) -> Result<(), IDTTransferManagerErr> {
        ...
        bindings.insert(vector, IrqBinding { pinned_task_id: task_id, port: port_arc, key });
        Ok(())
    }

    pub fn bind_isr(&self, task_id: TaskId, port_arc: Arc<Port>, key: u64) -> Result<(), IDTTransferManagerErr> {
        let mut bindings = self.bindings.write();
        if bindings.contains_key(&Self::ISR_SLOT) {
            return Err(IDTTransferManagerErr::AlreadyBound);
        }
        bindings.insert(Self::ISR_SLOT, IrqBinding { pinned_task_id: task_id, port: port_arc, key });
        Ok(())
    }
    ...
    pub fn unbind_task(&self, task_id: TaskId) {
        self.bindings.write().retain(|_, b| b.pinned_task_id != task_id);
    }

    pub fn get_isr(&self) -> Option<(TaskId, Arc<Port>)> {
        self.bindings.read().get(&Self::ISR_SLOT)
            .map(|b| (b.pinned_task_id, b.port.clone()))
    }

    pub fn get_irq(&self, vector: u8) -> Option<(TaskId, Arc<Port>, u64)> {
        self.bindings.read().get(&vector)
            .map(|b| (b.pinned_task_id, b.port.clone(), b.key))
    }
